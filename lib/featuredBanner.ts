...
export const newFeaturedBannerValidationSchema = commonValidationFeaturedBanner.extend({
  file: z
    .instanceof(File, { message: "Arquivo é obrigatório" })
    .refine((file) => ["image/jpeg", "image/png", "image/gif"].includes(file.type), {
      message: "Formato de arquivo inválido. Apenas imagens são permitidas.",
    }),
});

// Esquema para atualização de banners (arquivo opcional)
export const oldFeaturedBannerValidationSchema = commonValidationFeaturedBanner.extend({
  file: z
